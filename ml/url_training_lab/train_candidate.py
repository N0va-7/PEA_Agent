#!/usr/bin/env python3
"""Train a selected URL model candidate in an isolated experiment directory."""
...
from ml.url_training_lab.benchmark import build_candidates

DEFAULT_URL_CSV = PROJECT_ROOT / "ml" / "training" / "email_url" / "phishing_site_urls.csv"
DEFAULT_OUTPUT_DIR = PROJECT_ROOT / "ml" / "url_training_lab" / "artifacts"


def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Train one URL candidate model.")
    parser.add_argument("--candidate", choices=sorted(build_candidates().keys()), default="hybrid_char_lex_logreg")
...
def main() -> None:
    args = parse_args()
    urls, labels = load_dataset(args.csv, args.max_rows, args.seed)
    ...
    model = build_candidates()[args.candidate]
    ...
    final_model = build_candidates()[args.candidate]
